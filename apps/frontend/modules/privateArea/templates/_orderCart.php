...
$(function() {
	...
	$('a#orderNext').click(function() {
		...
		$('input[name=cartForm]').each(function(index, element){
			cartCounts.push(new Array(
								element.id,
								$(element).val()));
			});
		$.ajax({
			  type: 'POST',
			  url: '<?php echo url_for('@cart_save_count')?>',
			  data: ({cart: cartCounts}),
			  dataType: 'json',
			  cache: false,
...
<table class="cart_list" border="0" cellspacing="0" cellpadding="0">
  ...
  <tbody>
    ...
    <?php //<tr id="<?php echo 'cart_item_'.$company->getId().'_'.$cartItem->getId() " class="cart_item"> ?>
    <tr id="<?php echo 'cart_item_'.$cartItem->getId() ?>" class="cart_item">
    <td>
    ...
    <td class="cart_count">
      <form action="">
        ...
        <input id="<?php echo $cartItem->getId() ?>" name="cartForm" type="text" size="7" maxlength="2" value="<?php echo $cartItem->getCount() ?>" style="text-align: center;"/>
  ...
  <tfoot>
    <tr class="cart_sum">
      ...
      <td><a href="<?php echo url_for('@order_delivery')?>" id="orderNext"><img width="113" height="27" src="/images/privateArea/button_next.png" alt="Далее" style="cursor: pointer; margin: -10px -20px;" /></a></td>
    </tr>
    <!-- Доставка пока не доступна -->
    <!--
    <tr>
